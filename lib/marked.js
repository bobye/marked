...
inline.cite = replace(inline.cite)
  ('db', /dblp|gscholar|arxiv/)
  ();
...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.cite.exec(src)) {
      ...
      db = cap[1];
      key = cap[2];
      out += '(<span class="entry" db="' + db + '" key="' + key +'">'
	    + db +':'+ key
	    + '<span class="description">' + db + ': server connection lost</span>'
            + '</span>)';
      continue;
    }
...
>>>>>>> e2dcf5cb18c94890f79b06e9a21283c470043b30
    // code
    if (cap = this.rules.code.exec(src)) {
      src = src.substring(cap[0].length);
